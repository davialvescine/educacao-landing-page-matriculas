...
const PINS: {
  ...
  { slug: "goias", rotulo: "Goiás", x: 1270, y: 1230, delay: "1.2s", largura: 130 },
  { slug: "distrito-federal", rotulo: "DF", x: 1560, y: 1010, delay: "1.6s", largura: 88 },
  ...
  return (
    <Link href={`/${pin.slug}`} aria-label={`${pin.rotulo} — ${contagem} unidades`}>
      <g className="mapa-pin" style={{ cursor: "pointer" }}>
        <circle className="pin-pulso" cx={pin.x} cy={pin.y} r="36" fill="#f8c038" style={{ "--delay": pin.delay } as React.CSSProperties} />
        <circle cx={pin.x} cy={pin.y} r="22" fill="#f8c038" stroke="#050c42" strokeWidth="8" />
        <rect x={pin.x - (pin.largura * 1.35) / 2} y={pin.y + 42} rx="34" width={pin.largura * 1.35} height="68" fill="#050c42" opacity="0.92" />
        <text x={pin.x} y={pin.y + 89} textAnchor="middle" fontSize="38" fontWeight="800" fill="#ffffff" fontFamily="inherit">
          {pin.rotulo}
        </text>
      </g>
    </Link>
        ...
        <div className="mt-14 grid items-center gap-12 lg:grid-cols-12">
          <Reveal delay={0.1} className="lg:col-span-7">
            <svg
              ...
              className="mx-auto w-full max-w-[560px] font-sans"
            >
              <Forma chave="mt" />
          ...
          <Reveal delay={0.2} className="lg:col-span-5">
            <div className="flex flex-col divide-y divide-white/10 rounded-2xl border border-white/15 bg-brand-900/40 backdrop-blur-sm">
              {estados.map((e) => (
                <Link
                  key={e.slug}
                  href={`/${e.slug}`}
                  className="group flex items-center gap-4 px-6 py-4 transition-colors first:rounded-t-2xl last:rounded-b-2xl hover:bg-white/5"
                >
